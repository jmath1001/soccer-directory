'use client';

import React, { useEffect, useState } from 'react';
import { supabase } from '@/lib/supabaseClient';
import { useRouter } from 'next/navigation';
import { useAuthUser } from '@/hooks/useAuthUser';
import Link from 'next/link';
import type { RentalField } from '@/types';

const ADMIN_EMAIL = 'opatmath@gmail.com';

const RentalFieldsManage: React.FC = () => {
  const [fields, setFields] = useState<RentalField[]>([]);
  const [loading, setLoading] = useState(true);
  const [user, authLoading] = useAuthUser();
  const router = useRouter();

  useEffect(() => {
    if (!authLoading) {
      if (!user || user.email !== ADMIN_EMAIL) {
        router.push('/');
      }
    }
  }, [user, authLoading, router]);

  useEffect(() => {
    const fetchFields = async () => {
      try {
        const { data, error } = await supabase.from('rental_fields').select('*');
        if (error) throw error;
        setFields(data ?? []);
      } catch (error) {
        console.error('Error fetching fields:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchFields();
  }, []);

  const handleDelete = async (fieldId: string) => {
    const confirmDelete = window.confirm('Are you sure you want to delete this field?');
    if (!confirmDelete) return;

    const { error } = await supabase.from('rental_fields').delete().eq('id', fieldId);
    if (error) {
      console.error('Error deleting field:', error);
    } else {
      setFields((prev) => prev.filter((f) => f.id !== fieldId));
    }
  };

  if (loading || authLoading) return <div className="p-6">Loading...</div>;

  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6">Manage Rental Fields</h1>
      <div className="grid gap-4">
        {fields.map((field) => (
          <div key={field.id} className="border p-4 rounded shadow flex justify-between items-center">
            <div>
              <h2 className="text-xl font-semibold">{field.facility_name}</h2>
              <p className="text-gray-600">{field.location}</p>
            </div>
            <div className="flex gap-2">
              <Link
                href={`/edit-field/${field.id}`}
                className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded"
              >
                Edit
              </Link>
              <button
                onClick={() => handleDelete(field.id)}
                className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded"
              >
                Delete
              </button>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
};

export default RentalFieldsManage;
